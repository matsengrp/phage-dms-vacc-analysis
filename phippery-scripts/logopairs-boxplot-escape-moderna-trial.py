...
d_36 = set(id_coordinate_subset(
    ...
    is_equal_to="36 Days post-vaccination"
))
group1 = set(d_36)

d_119 = set(id_coordinate_subset(
    ds, where="visit_number",
    is_equal_to="119 Days post-vaccination"
))
group2 = set(d_119)

all_comparison_samples = set.union(group1, group2)
batch_samples = set(id_coordinate_subset(ds, where="library_batch", is_equal_to=f"{batch}"))
...
for epitope, metadata in EPITOPES.items():
    ...
    n_g1 = len(id_coordinate_subset(
        ds_cur, where="visit_number", is_equal_to="36 Days post-vaccination"
    ))
    n_g2 = len(id_coordinate_subset(
        ds_cur, where="visit_number", is_equal_to="119 Days post-vaccination"
        ))
    ...
    paired_participants = paired_participants[1:11]
    ...
    axd[mosaic[0,0]].set_title("36 days post vaccination")
    axd[mosaic[0,5]].set_title("119 days post vaccination")
    ...
    for i, (pid, pid_ds) in enumerate(iter_sample_groups(ds_paired, "participant_ID")):
        ...
        p1 = pid_tall[pid_tall["visit_number"]=="36 Days post-vaccination"]
        p2 = pid_tall[pid_tall["visit_number"]=="119 Days post-vaccination"]
    ...
    replicates_full["visit_number"] = replicates_full["visit_number"].cat.reorder_categories(
            ["36 Days post-vaccination", "119 Days post-vaccination"]
    )
    ...
    plt.savefig(f"{sys.argv[2]}-{epitope}")
